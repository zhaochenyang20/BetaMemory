...
def get_samples(task, x, y, n_generate_sample, prompt_sample, stop,knn=None):
    unique_trajectories = get_unique_trajectories(failed_trajectories)
    global reflection_map
    reflection_map = []
...
def generate_new_states(node, args, task,knn=None):
    ...
    sampled_actions = get_samples(task, prompt, f"Thought {node.depth + 1}: ", args.n_generate_sample, prompt_sample=args.prompt_sample, stop="Observation",knn=knn)
    ...
    for action in sampled_actions:
        ...
        if action_line:
            ...
            if new_node.is_terminal and r ==1:
                trajectory = collect_trajectory(new_node)
                success_trajectories.append({'trajectory': trajectory, 'final_answer': f"{action_type.lower()}[{action_param}]"})
