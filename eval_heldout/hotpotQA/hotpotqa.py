...
class HotPotQATask(Task):
    ...
    @staticmethod
    def cot_prompt_wrap(x: str, y: str = "", reflection_mapping_list=[], knn=None):
        question = x
        input = x + y
        trajectories = ""
        # z = []
        if reflection_mapping_list:
            ...
            if get_token_length(prompt) > max_token_length:
                ...
                for reflection_mapping in reflection_mapping_list[:3]:
                    ...
                    trajectories += traj_with_reflection
                prompt = cot_prompt_feedback_short.format(
                    trajectories=trajectories, input=input
                )
    ...
    def value_prompt_wrap(x: str, y: str, z: list = [], reflections: list = []) -> str:
        ...
        if len(z) != 0:
            ...
            prompt = value_prompt_reasoning_feedback.format(
                s="", trajectories=failed_trajectories, input=inp
            )

            if get_token_length(prompt) > max_token_length:
                prompt = value_prompt_reasoning_feedback_short.format(
                    s="", trajectories=failed_trajectories, input=inp
                )
        if len(z) != 0 and False:
            ...
            prompt = value_prompt_feedback.format(
                s="", trajectories=failed_trajectories, input=inp
            )
            if get_token_length(prompt) > max_token_length:
